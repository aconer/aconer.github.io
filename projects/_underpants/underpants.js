...
*   _.identity({a: "b"}) === {a: "b"}
*/
//functions on properties as objects
...
_.reject = function(array, funct){
    return _.filter(array, function(element, index, array){
     return !funct(element, index, array)
    });
}
...
_.partition = (array, funct) => 
    [_.filter(array, funct), _.reject(array, funct)];
...
_.map = (co, fn) => {
    let newArray = [];
    _.each(co, (element, index, collection) => {
    newArray.push(fn(element, index, collection))    
    })
    return newArray;
}
...
_.every = (co, fn) => {
    ...
    _.each(co, function (value, i, co) {
        fn(value, i, co) === false ? result = false: result;
    });
    return result
}
...
_.some = (co, fn) => {
    ...
    _.each(co, function(element, i, array){
        fn(element, i, array) === true ? Yes = true: Yes;
        });
    return Yes;
}
...
_.reduce = (col, fn, seed) => {
    let prev;
    if(seed !== undefined){
        prev = seed;
    _.each(col, (el, i, col) => prev = fn(prev, el, i));
    }
    else{
        prev = col[0];
        _.each(col, (el, i, col) => {
